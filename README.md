...
- **Batch folder:** `output_sheets/<batch>/pricing_hotels_all.csv` – generated by roomberg
- **CSV columns:** `hmid`, `check_in`, `price` (others are ignored)
- **Summary file (optional):** `reference_sheets/kuoni_hotel_summary.csv` with
...
- **Optional city lookup:** If `reference_sheets/kuoni_hotel_summary.csv` exists
  at the repo root, it maps `hmid` to `city`.

## Project layout
- `web/` - Next.js app (App Router).
- `output_sheets/` - Batch data directory read by the app.
- `reference_sheets/` - Reference data (city lookup).
